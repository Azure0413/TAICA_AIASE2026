...
def check_pairwise_role(rep: Report) -> None:
    ...
    if not p.exists():
        ...
        return
    text = _read(p)
    # Canonical format (course PAIRWISE_ROLE rules): a `roles:` list with one
    # `- role:` + `skill_path:` pair per role. Both code-author and bug-hunter must
    # be declared, with no duplicates (rule 1 & 2).
    pairs = re.findall(
    ...
    for role, sp in pairs:
        if role in roles:
            dup = True
        roles[role] = sp.rstrip("/")
    if dup:
        rep.add("pairwise-role:role", False, "duplicate role declared (rule 2)")
        return
    if set(roles) != {"code-author", "bug-hunter"}:
        rep.add("pairwise-role:role", False,
                f"must declare both code-author and bug-hunter; got {sorted(roles) or 'none'}")
        return
    rep.add("pairwise-role:role", True, "code-author + bug-hunter")
    for role, sp in sorted(roles.items()):
        if not (REPO_ROOT / sp).is_dir():
            rep.add("pairwise-role:skill_path", False, f"{role}: path not a directory: {sp}")
            return
    rep.add("pairwise-role:skill_path", True,
            ", ".join(f"{r}->{sp}" for r, sp in sorted(roles.items())))
...
def check_no_absolute_paths(rep: Report) -> None:
    ...
    for path in REPO_ROOT.rglob("*"):
        ...
        # TAICA2026_FinalProject.html 是課程規格書,僅「引用」禁用路徑作為反例,非繳交產物)
        if path.name in ("verify_repo.py", "hermes-config.example.yaml", "README.md",
                         "TAICA2026_FinalProject.html", "verify_report.json"):
            continue
        text = _read(path)
...
# 課程 LiteLLM Gateway 的 base_url 屬課程機密,不可被 commit。這裡用「通用」pattern 偵測任何
# 指向真實 https 主機的 base_url(刻意不寫死課程 URL,以免反而把它嵌進本檔)。placeholder
# 形式(base_url: https://<...>/v1)不會被誤判。
GATEWAY_URL_RE = re.compile(r"base_url\s*[:=]\s*['\"]?https://(?!<)[A-Za-z0-9.\-]+", re.IGNORECASE)


def check_no_gateway_url(rep: Report) -> None:
    offenders: list[str] = []
    for path in REPO_ROOT.rglob("*"):
        if not path.is_file():
            continue
        ...
        if path.suffix in (".sqlite", ".sqlite3", ".db", ".png", ".jpg", ".pdf"):
            continue
        # 規格書(僅引用範例)與本檔(含 regex)不計入
        if path.name in ("verify_repo.py", "TAICA2026_FinalProject.html"):
            continue
        for m in GATEWAY_URL_RE.finditer(_read(path)):
            offenders.append(f"{path.relative_to(REPO_ROOT)}: {m.group(0)[:50]}")
    rep.add("no-gateway-url", not offenders,
            "" if not offenders else
            f"real base_url committed (課程機密,請改 placeholder); first: {offenders[0]}")
