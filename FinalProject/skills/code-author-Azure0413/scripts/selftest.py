...
def _raw_payload(argv: list[str]) -> str:
    """argv[1], or stdin when argv absent/'-' (quote-safe heredoc path; isatty()-guarded)."""
...
def main(argv: list[str]) -> int:
    ...
    if not raw.strip():
        return _emit({"passed": 0, "failed": 0, "errors": ["usage: selftest.py '<json>' (or stdin)"],
                      "sloc": 0, "loc_violation": False, "import_violations": []})
    try:
        payload = json.loads(raw)
    except json.JSONDecodeError as e:
        return _emit({"passed": 0, "failed": 0, "errors": [f"payload JSON invalid: {e}"],
                      "sloc": 0, "loc_violation": False, "import_violations": []})
