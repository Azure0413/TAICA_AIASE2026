...
Usage:
    ...
    # `db_schema` is also accepted as an alias for `schema_ddl` (matches the task input contract).
...
def _raw_payload(argv: list[str]) -> str:
    """argv[1], or stdin when argv is absent/'-' (quote-safe heredoc path; see run.py).
    isatty()-guarded so it never blocks on an interactive terminal."""
...
def main(argv: list[str]) -> int:
    ...
    if not raw.strip():
        return _emit(False, "usage: validate_sql.py '<json>'  (or pipe JSON on stdin)")
    try:
        payload = json.loads(raw)
    except json.JSONDecodeError as e:
        return _emit(False, f"payload JSON invalid: {e}")
    # Accept either `schema_ddl` (this script's own key) or `db_schema` (the key used by the
    # task input contract). LLMs frequently reuse the input field name `db_schema`; reading only
    # one key would silently validate against an empty schema and wrongly report "no such table".
    schema = payload.get("schema_ddl")
    if not schema:
        schema = payload.get("db_schema", "")
    ok, err = validate(str(schema or ""), str(payload.get("sql", "")))
    return _emit(ok, err)
