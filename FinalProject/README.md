...
# 正式評分指令(課程 2026-06 file-based 更新):skills,terminal 兩個 toolset 缺一不可(terminal 用來跑
# scripts/),--yolo 全自動放行工具呼叫,-q 非互動式,-Q 防止 hermes 美化訊息吃掉 ``` 標記。
# 輸出改為 file-based:skill 由 scripts/run.py 把結果寫進 $AIASE_RESULT_PATH(未設則 ./aiase_result.json),
# 評分器讀檔評分。本地自測請用同一條,與評分環境一致。
AIASE_RESULT_PATH=./aiase_result.json \
  hermes chat --toolsets skills,terminal --yolo -Q -q '/hello-aiase {"task_id":"t","name":"world"}'
cat ./aiase_result.json   # 應看到 hello-aiase 寫出的結果 JSON
...
`run_dev.py`(file-based 版)會對每題設好 `AIASE_RESULT_PATH`、用含 `-Q` 的正式指令呼叫你的 skill、再用
repo 根目錄的 `aiase_contract.py`(與評分器**同一份**比對核心)**讀結果檔**比對,並把彙整結果寫進
`dev_run_results/`。看到「讀到結果檔 + 有比對結果」就代表你已接上 file-based;一直顯示「no result file」=
skill 還沒改成寫檔。
...
├── run_dev.py                             ← 本地自測:驅動 hermes chat -Q + 讀結果檔比對
├── aiase_contract.py                      ← 評分器與 run_dev 共用的比對核心(file-based,勿改)
...
2. **輸出契約(file-based,2026-06 更新)**:每個 skill 的最後一個動作 = 用 `terminal` 執行 `scripts/run.py`,
   把唯一一份契約 JSON **寫進結果檔**(`$AIASE_RESULT_PATH`,否則 `./aiase_result.json`);評分器**讀檔**評分,
   不再從對話 stdout 擷取 JSON。**沒有結果檔 = 該題 0 分**;不需在對話再輸出 JSON。
