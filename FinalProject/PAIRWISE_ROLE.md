...
roles:
  - role: code-author
    skill_path: skills/code-author-Azure0413/
  - role: bug-hunter
    skill_path: skills/bug-hunter-Azure0413/
```
...
3. 每個 `skill_path` 必須指向實際存在、能被 `hermes skills list` 看到的 skill 資料夾。
...
<!-- 兩個 skill 皆已通過格式 gate 與本地自測，可被 hermes skills list 看到並以名稱觸發。 -->
<!-- 配對對象由課程自動隨機指派，不得私下約定（學術誠信 §六）。 -->
