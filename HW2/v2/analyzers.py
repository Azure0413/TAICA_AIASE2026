...
class ScanEngine:
    ...
    def scan_path(self, path, exclude_patterns=None):
        """掃描指定路徑，支援排除模式。"""
        py_files = self._collect(path, exclude_patterns=exclude_patterns)
    ...
    def _collect(self, path, exclude_patterns=None):
        """蒐集 .py 檔案，支援排除模式（v2.0 新增）。"""
        ...
        for root, dirs, files in os.walk(path):
            ...
            # v2.0：依排除規則過濾目錄
            if exclude_patterns:
                dirs[:] = [d for d in dirs if not self._matches_any(d, exclude_patterns)]
            for fn in sorted(files):
                if not fn.endswith(".py"):
                    continue
                rel_path = os.path.relpath(os.path.join(root, fn), path)
                # v2.0：依排除規則過濾檔案
                if exclude_patterns and self._path_matches_any(rel_path, exclude_patterns):
                    continue
    ...
    def _matches_any(name, patterns):
        """檢查名稱是否符合任一排除模式（支援 fnmatch 萬用字元）。"""
        return any(fnmatch.fnmatch(name, p) for p in patterns)

    @staticmethod
    def _path_matches_any(rel_path, patterns):
        """檢查相對路徑中的任何部分是否符合排除模式。"""
