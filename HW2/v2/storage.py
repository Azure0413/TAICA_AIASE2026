...
class JsonStorage(BaseStorage):
    ...
    def update_checklist_status(self, scan_id, index, resolved):
        """v2.0：更新指定掃描的交接問題清單確認狀態。

        確認狀態直接儲存在 ScanResult 的 checklist_status 欄位中，
        與原有資料結構一起持久化，不另開檔案。
        """
        recs = self._read()
        for r in recs:
            if r["id"] == scan_id:
                checklist = r.get("question_checklist", [])
                if not checklist:
                    return False
                statuses = r.get("checklist_status", [False] * len(checklist))
                # 確保長度足夠
                while len(statuses) < len(checklist):
                    statuses.append(False)
                if index < 0 or index >= len(checklist):
                    return False
                statuses[index] = resolved
                r["checklist_status"] = statuses
                self._write(recs)
                return True
        return False
